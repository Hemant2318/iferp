...
import { sendRequests, fetchUserNetwork } from "store/globalSlice";
import {
    ...
    <div className="shadow mt-4">
      ...
      <div className="cps-20 cpt-20 cpe-20 cpb-20 d-flex flex-column gap-3">
        {isPageLoading ? (
          ...
          following?.slice(0, 4)?.map((elem, index) => {
            ...
            return (
              <div className="fa-center gap-2" key={index}>
                ...
                {!isHide && (
                  <div>
                    <Button
                      btnText={isExist ? "Follow" : "Unfollow"}
                      btnStyle="GO"
                      className={`h-38 ${isExist ? "ps-3 pe-3" : "ps-4 pe-4"}`}
                      onClick={() => {
                        handelSendRequest({
                          id,
                          isExist,
